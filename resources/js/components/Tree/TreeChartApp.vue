    ...
    <div class="popup" ref="popup_wrapper">
      <div class="popup-wrapper">
        ...
        <div>
          <h2>Видео:</h2>
          <div
            v-for="item in infoTechnologyVideo"
            v-bind:key="item.id"
            class="video-link"
          >
...
export default {
  ...
  mounted() {
    ...
    document.querySelector("body").addEventListener("click", (e) => {
      if (e.target.closest(".node")) {
        console.log(
          e.target.closest(".node").querySelector(".name").textContent
        );
        const nameTechnology = e.target
          .closest(".node")
          .querySelector(".name")
  ...
  methods: {
    loadInfo(nameTechnology) {
      axios.get(`/technology/${nameTechnology}/export`).then((res) => {
